...
-- User data
local user = "nicholas.fose" -- Todo:
local pass = "ee8WsFMWds9Tuhw" --  * find a safer storage

-- {{{ Gmail widget type
local function worker(format, feed)
    local auth = user .. ":" .. pass
    local feed = feed or "https://mail.google.com/mail/feed/atom"
    local mail = {
        ["{count}"]   = "0",
        ["{subject}"] = "N/A"
    }
    ...
    local f = io.popen("curl --connect-timeout 1 -n -m 3 -fsu "..auth.." "..feed)
